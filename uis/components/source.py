...
from facefusion.filesystem import filter_audio_paths, filter_image_paths, has_audio, has_image, resolve_relative_path
...
SOURCE_BROWSE_BUTTON : Optional[gradio.Button] = None
...
def render() -> None:
	...
	global SOURCE_AUDIO
	global SOURCE_IMAGE
	global SOURCE_BROWSE_BUTTON

	swap_dir = resolve_relative_path('swap')
	...
	SOURCE_BROWSE_BUTTON = gradio.Button(
		value = wording.get('uis.source_browse_button'),
		variant = 'secondary',
		size = 'sm'
	)
	...
	register_ui_component('source_browse_button', SOURCE_BROWSE_BUTTON)


def listen() -> None:
	SOURCE_FILE.change(update, inputs = SOURCE_FILE, outputs = [ SOURCE_AUDIO, SOURCE_IMAGE ])
	if SOURCE_BROWSE_BUTTON:
		SOURCE_BROWSE_BUTTON.click(browse_swap_directory)
...
def browse_swap_directory() -> None:
	import subprocess
	import platform
	...
	if platform.system() == 'Windows':
		subprocess.run(['explorer', swap_dir], check=False)
	elif platform.system() == 'Darwin':  # macOS
		subprocess.run(['open', swap_dir], check=False)
	else:  # Linux
		subprocess.run(['xdg-open', swap_dir], check=False)
